...
void While::Gen()
{
    unsigned start = NewLabel();   // Rótulo de início do loop
    unsigned end = NewLabel();     // Rótulo de saída do loop

    cout << "L" << start << ":" << endl;  // Marca início do loop

    Expression *cond = Rvalue(expr);  // Avalia a condição do while
    cout << "\tifFalse " << cond->ToString() << " goto L" << end << endl;

    stmt->Gen();  // Gera código para o corpo do loop

    cout << "\tgoto L" << start << endl;  // Volta para o início do loop
    cout << "L" << end << ":" << endl;  // Marca fim do loop
}
